...
mod tests {
    ...
    #[test]
    fn named_arguments_are_sorted_before_codegen() {
        let args: FlArgs = syn::parse_str(", right = 2, left = 1").unwrap();
        let FlArgs::KeyValuePairs { specified_args } = args else {
            panic!("expected named arguments");
        };
        let keys: Vec<String> = specified_args
            .iter()
    ...
    fn duplicate_named_arguments_are_rejected() {
        assert!(syn::parse_str::<FlArgs>(", left = 1, left = 2").is_err());
    }
}
